...
function validateMenuItemSelection(event) {
	...
	switch (collapsible.id) {
		case "type1":
		case "type2":
			checkBoxes = $(meals).find(`input[type="checkbox"]`).filter((index, checkbox) => checkbox.checked);
	...
	switch (collapsible.id) {
		case "type1":
			var checkBoxes = $(additions).find(`input[type="checkbox"]`).filter((index, checkbox) => checkbox.checked);
			...
			break;
		case "type2":
			var checkBoxes = $(types).find(`input[type="checkbox"]`).filter((index, checkbox) => checkbox.checked);
...
function addMenuItemToOrder(event) {
	...
	for (var i = 0; i < checkBoxes.length; i++) {
		...
		if (checkBox.checked) typeFromMenu = $(types).find(`label[for='${checkBox.id}']`)[0].innerText;
	}
	...
	for (var i = 0; i < checkBoxes.length; i++) {
		var checkBox = checkBoxes[i];
		//inputsToClear.push(checkBox);
		if (checkBox.checked) mealsFromMenu.push($(meals).find(`label[for='${checkBox.id}']`)[0].innerText);
	}
	...
	for (var i = 0; i < checkBoxes.length; i++) {
		var checkBox = checkBoxes[i];
		//inputsToClear.push(checkBox);
		if (checkBox.checked) additionsFromMenu.push($(additions).find(`label[for='${checkBox.id}']`)[0].innerText);
	}
	...
	for (var i = 0; i < checkBoxes.length; i++) {
		var checkBox = checkBoxes[i];
		//inputsToClear.push(checkBox);
		if (checkBox.checked) saucesFromMenu.push($(sauces).find(`label[for='${checkBox.id}']`)[0].innerText);
	}
	...
	for (var i = 0; i < checkBoxes.length; i++) {
		var checkBox = checkBoxes[i];
		//inputsToClear.push(checkBox);
		if (checkBox.checked) drinksFromMenu.push($(drinks).find(`label[for='${checkBox.id}']`)[0].innerText);
	}
