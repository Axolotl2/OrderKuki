...
function addRefreshAndExitHandler() {
  ...
  window.manageBackRefresh = function (event) {
    ...
    if (
      ...
      var backOk =
        oOrder.keys.length == 0 ||
        confirm(`יציאה תמחק את טבלת ההזמנות, להמשיך?`);
      if (backOk) {
      ...
      var refreshOk =
        Object.keys(oOrder).length == 0 ||
        confirm(`ריענון ימחק את טבלת ההזמנות, להמשיך?`);
      if (refreshOk) {
        window.landg.innerDocClick = true;
      } else {
        event.preventDefault();
  ...
  window.onbeforeunload = function (event) {
    ...
    var ele = $(":focus");
    if (
      !window.landg.innerDocClick &&
      ($(ele) == undefined ||
        $(ele).attr("href") == undefined ||
        ($(ele).attr("href") != undefined && $(ele).attr("href") != "#"))
    ) {
